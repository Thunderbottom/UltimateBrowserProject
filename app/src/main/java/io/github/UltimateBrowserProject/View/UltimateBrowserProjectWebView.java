...
public class UltimateBrowserProjectWebView extends WebView implements AlbumController {
    ...
    private final int UP_SCROLL_THRESHOLD = convertDpToPixels(1); // Change maybe
    private final int DOWN_SCROLL_THRESHOLD = convertDpToPixels(100);
    private float y1;
    private float y2;
        ...
        setOnTouchListener(new OnTouchListener() {
            ...
            public boolean onTouch(View view, MotionEvent motionEvent) {
                ...
                if (action == MotionEvent.ACTION_DOWN) {
                    ...
                    if ((y1 - y2) > DOWN_SCROLL_THRESHOLD) {
                        browserController.showOmnibox();
                    } else if ((y1 - y2) < -UP_SCROLL_THRESHOLD) {
